      ...
      <main className="mx-auto max-w-3xl px-6 py-8">
          ...
          {editingInfo ? (
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                {conti.description && (
                  <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">{conti.description}</p>
                )}
              </div>
              <button
        ...
        {conti.songs.length === 0 ? (
          <div className="rounded-xl border-2 border-dashed border-gray-200 py-12 text-center dark:border-gray-700">
            ...
            <button
              onClick={handleOpenAddSong}
              className="mt-2 text-sm font-medium text-violet-500 hover:underline"
            >
              찬양 추가하기
            </button>
          </div>
          ...
          <div className="space-y-2">
            {conti.songs.map((cs, index) => (
              ...
              >
                {/* 순서 번호 */}
                <span className="w-6 shrink-0 text-center text-sm font-bold text-gray-300 dark:text-gray-600">
                  {index + 1}
                </span>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {cs.song.artist && (
                    <p className="text-xs text-gray-400">{cs.song.artist}</p>
                  )}
                  {cs.note && (
                    <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{cs.note}</p>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 sm:items-center sm:p-4">
          <div className="flex h-[80vh] w-full flex-col rounded-t-2xl bg-white dark:bg-gray-900 sm:h-auto sm:max-h-[80vh] sm:max-w-lg sm:rounded-2xl">
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-gray-800">
              ...
              <button
                onClick={() => { setShowAddSong(false); setSelectedSong(null); setAddingKey(''); setAddingNote(''); setSongSearch(''); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 dark:bg-gray-900">
            ...
            <form onSubmit={handleSaveCs} className="space-y-3">
              ...
              <div>
                <label className="mb-1 block text-sm text-gray-600 dark:text-gray-400">메모</label>
                <input
                  type="text"
                  value={editNote}
